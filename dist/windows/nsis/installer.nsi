#
# NSIS installer script for the VASSAL Extension Utility.
#
...
#
# Differences from the VASSAL installer, deliberately:
#
# - No file associations. VASSAL itself owns .vmod/.vlog/.vsav, and this
#   utility must never steal them.
# - The uninstall registry KEY is "VASSAL-Extension-Utility <version>"
#   (hyphenated). VASSAL's own installer enumerates keys by the 7-character
#   prefix "VASSAL " when offering to remove old versions — and removes them
#   ALL in a Standard install — so our key must not match that prefix, or
#   installing VASSAL would silently uninstall this utility.
#
...
!define APPEXE "VASSAL-Extension-Utility.exe"

!define UNINST "Software\Microsoft\Windows\CurrentVersion\Uninstall"
; the uninstall key: must NOT begin with "VASSAL " — see header comment
!define VNAME "VASSAL-Extension-Utility ${VERSION}"
...
!else
  InstallDir "$PROGRAMFILES\VASSAL-Extension-Utility-${VERSION}"
!endif
...
VIAddVersionKey ProductName "VASSAL Extension Utility"
VIAddVersionKey CompanyName vassalengine.org
VIAddVersionKey LegalCopyright "VASSAL Extension Utility contributors"
VIAddVersionKey FileDescription "VASSAL Extension Utility installer"
VIAddVersionKey FileVersion ${NUMVERSION}.0
VIAddVersionKey ProductVersion ${VERSION}
VIAddVersionKey InternalName vassal-extension-utility
...
!define MUI_STARTMENUPAGE_DEFAULTFOLDER "VASSAL Extension Utility"
...
check_processes:
  ...
  ${Do}
    ...
    ${If} $R7 == "${APPEXE}"
      ...
      MessageBox MB_OKCANCEL|MB_ICONEXCLAMATION "An instance of the VASSAL Extension Utility is currently running.$\n$\nPlease close it and press OK to continue, or Cancel to quit." IDCANCEL bail_out
      Sleep 500
...
cannot_check:
  MessageBox MB_OKCANCEL|MB_ICONEXCLAMATION "Unable to determine whether an instance of the VASSAL Extension Utility is running.$\n$\nPlease close all instances of the VASSAL Extension Utility before proceeding." IDCANCEL bail_out
  Return
...
; VASSAL's own "VASSAL <version>" keys, and VASSAL's 7-character "VASSAL "
; check never matches ours.
...
Function .onInit
  ...
  ${If} "$0" != "${ARCH_PRETTY}"
    ...
    MessageBox MB_OK|MB_ICONEXCLAMATION "This installer requires ${ARCH_PRETTY} Windows.$\n$\nTo use the VASSAL Extension Utility ${VERSION} on $0 Windows, please install the $0 Windows package."
    Abort
  ${EndIf}
...
Function preSetupType
  ...
  ${NSD_CreateLabel} 30u 37u 100% 12u "The VASSAL Extension Utility will be installed with the most common options."
	Pop $0
...
Function preUninstallOld
  ...
  !insertmacro MUI_HEADER_TEXT "Remove Old Versions" "Uninstalling previous versions of the VASSAL Extension Utility"

  nsDialogs::Create /NOUNLOAD 1018
  Pop $0

  ${NSD_CreateLabel} 0 0 100% 24u "The installer has found these other versions of the VASSAL Extension Utility installed on your computer. Please select the versions you would like to remove now."
  Pop $0
...
Function preShortcuts
  ...
  ${NSD_CreateLabel} 0 0 100% 12u "Create icons for the VASSAL Extension Utility:"
  Pop $0
...
Function preConfirm
  ...
  ${NSD_CreateLabel} 0 0 100% 100% "The installer is ready to install the VASSAL Extension Utility on your computer.$\n$\n$\nClick $\"Install$\" to start the installation."
  Pop $0
...
Section "-Application" Application
  ...
  WriteRegStr HKLM "${UROOT}" "DisplayName" "VASSAL Extension Utility ${VERSION}"
  ...
  WriteRegStr HKLM "${UROOT}" "URLInfoAbout" "https://github.com/delatbabel/vassal_extension_utility"
  WriteRegStr HKLM "${UROOT}" "URLUpdateInfo" "https://github.com/delatbabel/vassal_extension_utility"
  ...
  ${If} $CustomSetup == 1
    StrCpy $0 "VASSAL Extension Utility ${VERSION}"
  ${Else}
    StrCpy $0 "VASSAL Extension Utility"
  ${EndIf}
  ...
  ; no file associations: VASSAL itself owns .vmod/.vlog/.vsav
SectionEnd
...
Section Uninstall
  ...
  RMDir "$SMPROGRAMS\VASSAL Extension Utility"
