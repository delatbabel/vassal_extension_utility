...
another game's maps from the Tools menu; the window has a light and a dark
theme; saved games in VASSAL 3.8's new obfuscation format open; the
command-line tools run again, and gain one for the game state that no counter
accounts for.

### Added

- **Light and dark themes — View → Theme.** The application had only the platform look and feel, which offers nothing dark and, on Linux, usually means Metal. It now ships [FlatLaf](https://www.formdev.com/flatlaf/)'s plain `FlatLightLaf` and `FlatDarkLaf` — the neutral pair, not the IDE-flavoured variants — with **Light** as the default, and keeps the platform look as a third choice, **System**, for anyone who wants the window to match the rest of their desktop. A change applies to every open window at once, with no restart, and is remembered in `~/.vassal-extension-utility/ui.properties`. The greys that mark something present but secondary — an inactive extension, an inherited (module) tree node, a map whose layout will not change — were a hard-coded `Color.GRAY` that all but vanished on a dark background; they and the coloured panel borders now come from the theme and are read at paint time, so they follow a switch instead of being fixed at startup. FlatLaf needs no JDK module the packaged runtimes do not already link, and its native libraries travel inside the jar.
