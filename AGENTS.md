...
| `model/SavedGame` | Opens a `.vsav` saved game (deobfuscates the `savedGame` command log), finds "excess" pieces missing from the module + active extensions, and rewrites the game without them (byte-exact) to a new file (see [Excess units in a saved game](#excess-units-in-a-saved-game) and **[docs/vsav-excess-units.md](docs/vsav-excess-units.md)**) |
...
| `gui/MainWindow` | Top-level `JFrame` — split pane of two `ArchivePanel`s, toolbar, status bar; the **Show Extensions** button lists/activates a module's extensions (see [Show extensions](#show-extensions)); the **Excess Units…** button finds & removes orphaned pieces from a `.vsav` (see [Excess units in a saved game](#excess-units-in-a-saved-game)) |
...
A **module** panel's tree is a direct DOM walk (`ArchivePanel.buildTreeNode`). An **extension** panel instead reconstructs the *module hierarchy* the extension grafts into, so its components appear where the editor would show them rather than as flat `ExtensionElement` rows (`ArchivePanel.buildExtensionTree` / `graftInto`):

- Each `ExtensionElement`'s `target` is decoded (`decodeTarget` → `seqDecode`, a faithful port of VASSAL's `SequenceEncoder.Decoder` — the exact inverse of the `seqToken`/`seqJoin` encoder in `MainWindow`) into a chain of `className:configureName` path segments.
- Those segments become **synthetic "inherited" nodes** — `ComponentNode`s built with the `ComponentNode(className, configureName)` constructor, which carry no DOM element (`isInherited()` / `getElement() == null`). Shared prefixes are merged (`findOrCreateInherited` matches on class + name), so two components targeting nearby module locations share the same path nodes, exactly as in the module tree.
- The `ExtensionElement`'s **real component subtree** hangs beneath its target's last node as normal (element-backed) `ComponentNode`s.
- A **nested** `ExtensionElement` (as left by the double-wrapping bug) is re-grafted from the root, since its target is an absolute module path — so even a doubly-wrapped extension displays as the correct hierarchy (and can still be fixed with [Repair double-wrapped extension elements](#repair-double-wrapped-extension-elements)).

Rendering distinguishes the two kinds: `ArchivePanel.InheritedAwareRenderer` (a `DefaultTreeCellRenderer`) paints **inherited (module) nodes grey** and the extension's own components in the normal colour (grey is applied only when the node is not selected, keeping the selection highlight legible). Inherited nodes are display-only stand-ins: they cannot be Moved, Copied, or Deleted (see [Move / Copy](#move--copy-operation-the-core-feature) and [Delete](#delete)), but they *can* be chosen as a Move/Copy **destination** to graft into that module location.

Because synthetic nodes have no `Element`, [tree-state preservation](#tree-state-preservation) keys on a generalized `keyOf` — the DOM `Element` for real nodes, or a structural `"SYN:" + className:name/…` path (`syntheticPath`) for inherited ones — both stable across a rebuild of the same archive.
...
The **Excess Units…** toolbar button and **Tools → Find Excess Units in Saved Game…** (`MainWindow.checkExcessUnits`) find and remove game pieces in a `.vsav` saved game whose definitions are missing from the module (left panel) and its **active** extensions — the pieces that make VASSAL log *"Bad Data in Module … Image not found"* / *"No such map"* on load and *"Unable to match piece … by name"* on **Tools → Refresh Counters**. It requires a module in the left panel; the user then picks a `.vsav`.

- **Model — `model/SavedGame`.** `SavedGame.open` deobfuscates the `savedGame` command log (`!VCSK` + XOR-hex, see [docs/vsav-format.md](docs/vsav-format.md)) into memory and splits it at **every** ESC into command tokens (recording each token's preceding-delimiter and content byte ranges — see below). It works entirely on the deobfuscated `byte[]`; the two metadata entries (`moduledata`, `savedata`) are read whole and kept.
- **Detection — `SavedGame.findExcessPieces`.** For each `AddPiece` command (`+/<id>/<type>/<state>`) whose innermost trait is a `BasicPiece` (`piece;<clone>;<delete>;<image>;<name>` in the type; `<map>;<x>;<y>;<gpid>;…` in the state), it extracts the piece's **GPID** (4th `;`-field of the innermost state) and **name** (5th `;`-field of the innermost type). A piece is **excess** iff its GPID matches no active PieceSlot **and** its name matches no active PieceSlot name — exactly VASSAL's `GpIdChecker` "unmatchable" condition (GPID first, then name fallback). Requiring **both** keys to fail is what makes it safe: a run-time-placed marker (fresh GPID but same name as a live definition) and a piece still matchable by GPID (repairable by Refresh Counters) are never removed. The image is shown in the report but is **not** a detection key — an orphaned piece whose GPID/name are gone is flagged even if its image happens to still exist (e.g. the sample's `germanyBPs`, GPID 10990). If a flagged piece's GPID or name is found in an **inactive** extension, that extension's name is shown in braces (grey) — activating it (Show Extensions) is the alternative to deleting.
- **Active/inactive definitions — `MainWindow.collectSlots`.** For the module and each `.vmdx` in `_ext`, it collects every `gpid` attribute and the innermost BasicPiece name of every `PieceSlot`/`CardSlot`/`PrototypeDefinition` (whose definition text is itself a `+/null/<type>/<state>` command, parsed by `SavedGame.basicPieceName`). The same over `_ext/inactive/` builds the GPID→ext and name→ext maps for the "(Inactive)" annotation. This runs, together with the deobfuscation, on a background `SwingWorker` behind a modal indeterminate progress dialog (`MainWindow.makeProgressDialog`) so the (large) file work never blocks the EDT and the user cannot close the window mid-operation.
- **Byte-exact rewrite — `SavedGame.saveWithout`.** The command log is a **two-level** `SequenceEncoder` structure: top-level commands separated by a bare ESC (`0x1B`), and the pieces grafted one level deeper separated by an escaped `\<ESC>` (VASSAL classifies an ESC as a delimiter iff its immediately-preceding byte is not `\`). Because piece data contains no ESC, splitting on every ESC and re-emitting the **same** delimiter bytes reconstructs the nesting exactly. To remove a piece, its token **and its preceding delimiter** are dropped and every surviving token is copied **verbatim** (never decoded/re-encoded), then the whole stream is re-obfuscated with a fresh key. The `moduledata`/`savedata` entries are copied byte-for-byte. The output is written to a **temp file and atomically moved into place** (like `VassalArchive.writeArchive`), so an interrupted or failed write never leaves a truncated `.vsav` at the target — a truncated file is exactly what makes VASSAL report *"… is not a VASSAL saved game or log."* Deobfuscation reads the whole entry at once and the obfuscated output is emitted in 64 KB chunks (not per byte), so a ~465 MB game rewrites in a few seconds. (Verified end-to-end on the sample: the tidied file reopens with exactly *original − removed* commands, zero excess on re-scan, byte-identical metadata, and passes `unzip -t`.)
- **Save flow.** After confirmation the user is prompted for a **new** file name (defaulting to `<name> (tidied).vsav`), and the original file is left untouched — per the requirement. Refusing to overwrite the original is enforced.
...
- **Identity key** (`keyOf`) — for a real component, the DOM `Element` object, which is stable across a rebuild of the *same* archive (`buildTreeNode` makes fresh `DefaultMutableTreeNode`/`ComponentNode` wrappers but reuses the underlying elements). For a synthetic **inherited** node (extension display — see [Extension tree display](#extension-tree-display)), which has no element, a structural `"SYN:" + className:name/…` path (`syntheticPath`) — likewise stable, since the same `ExtensionElement` targets rebuild the same synthetic hierarchy. After rebuild, the new tree is indexed `key → node` to translate captured state onto the new nodes.
...
1. All selected source nodes are collected via `src.getSelectedNodes()`. **Inherited (grey) nodes can't be sources** — they stand in for module components the extension only grafts into, have no element to transfer, and abort the operation with a status message (see [Extension tree display](#extension-tree-display)).
...
3. The destination parent is resolved:
   - **Grafting into an extension** (`graftIntoExtension`) — when the target archive is an extension *and* the destination is either its top level (`graftTopLevel`: nothing selected, or the `ModuleExtension` root selected) or a selected inherited node (`graftIntoInherited`): each component is wrapped in a `VASSAL.build.module.ExtensionElement`. An extension never holds components directly — they must sit inside an `ExtensionElement` whose `target` names the path, in the *parent module's* tree, to graft into. Appending a raw component to the extension root instead (the old behaviour) made VASSAL silently ignore it, so the component never appeared in the editor. The target is either `moduleTargetPath()` — built from the **source** element's parent chain, for a top-level graft (grafts each component back at its original module location) — or `syntheticTargetPath()` — built from the **destination** inherited node's own path, when the user picked a specific inherited location to graft into. Both emit `className:configureName` tokens encoded exactly like VASSAL's `ComponentPathBuilder`/`SequenceEncoder` ('/'-joined, ':'-joined, delimiters backslash-escaped; an empty target means graft at the module root). `createExtensionElement()` makes a **new wrapper for every component** — wrappers are **never shared, even for the same target** — because a VASSAL `ExtensionElement` holds exactly one component (`ExtensionElement.build()` builds only its first child element and its `add()` keeps a single `extension`). Packing several components into one wrapper (an earlier bug) made VASSAL silently keep only the first and drop the rest — and, if the dropped board owned an image, that image then never displayed even though its bytes/tiles were fine. This matches the editor, which writes one `ExtensionElement` per component. (Verified: the generated target string is byte-for-byte identical to the ones VASSAL itself writes into the sample Europa extensions.) **When the selected source element is itself an `ExtensionElement`** (defensive — the tree no longer shows wrappers as their own nodes since [Extension tree display](#extension-tree-display), but a raw wrapper could still be selected via other paths), it is already a valid top-level extension child and is grafted **directly onto the destination root — never re-wrapped**. Re-wrapping it (an earlier bug) produced a nested pair: an outer, target-less `ExtensionElement` (since `moduleTargetPath()` returns `""` — the source wrapper's parent is the extension root) around the real one; VASSAL loads it but the module editor cannot edit such a doubly-wrapped component. Extensions already damaged by that bug can be fixed with **Tools → Repair Double-Wrapped Extension Elements** (see [Repair double-wrapped extension elements](#repair-double-wrapped-extension-elements)).
   - Else if a real (element-backed) node **is** selected in the target panel, every source component is transferred into it (this covers adding into an existing grafted subtree inside an extension). (A selected *inherited* node routes to the graft path above, not here.)
   - Else (nothing selected, module target), an OK/Cancel dialog offers to recreate parent paths. On OK, `ensureAncestorPath()` shallow-clones each source component's ancestor chain (every parent from just below the source root down to the immediate parent) under the destination root, and the component is transferred into the recreated parent. `findMatchingChild()`/`sameElement()` (tag name + identical attributes) reuse equivalent existing destination elements so shared paths are consolidated and the source root maps onto the destination root. Cancel aborts. **Guard — a module must never contain an `ExtensionElement`:** if the source lives inside an extension's `ExtensionElement` (checked with `hasExtensionElementAncestor()`), recreating its ancestor chain would shallow-clone that wrapper into the module, producing an illegal module (it loads, but e.g. VASSAL's *Tools → Refresh Counters* rejects it with a misleading "saved with older version" error). The operation is refused up front with an error dialog telling the user to **select the intended parent component in the module tree first** (which routes to the "node is selected" case above, appending the component directly with no wrapper). The guard is scoped to this no-parent-chosen, module-destination path only, so legitimate module→module recreate-parents is unaffected.
...
2. The archive root (`GameModule`/`ModuleExtension`) can never be deleted — attempting it aborts with a status message instead of a confirm dialog. **Inherited (grey) nodes** likewise can't be deleted (they belong to the module, not the extension) and abort with a status message.
...
5. **In an extension**, deleting a grafted component removes it from its (unshown) `ExtensionElement` wrapper; a wrapper thereby left empty is dropped via `removeEmptyExtensionElements` (the same guard Move uses), because VASSAL crashes loading an empty `ExtensionElement` (see [docs/vassal-empty-extensionelement-crash.md](docs/vassal-empty-extensionelement-crash.md)).
6. The archive is marked modified, `panel.refresh()` rebuilds the tree (preserving expansion/selection/scroll for whatever remains — see [Tree state preservation](#tree-state-preservation)), and the deletion is only written to disk on the next Save.
